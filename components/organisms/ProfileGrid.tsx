      ...
      {profiles.map((profile, index) => {
        ...
        return (
          <motion.div
            ...
            className="group relative flex flex-col justify-between rounded-[2rem] border border-slate-200/80 bg-white p-3 shadow-sm hover:shadow-2xl hover:border-purple-300 transition-all duration-300"
          >
            <div>
              {/* Photo Avatar en Format Portrait Élégant avec Coins Arrondis */}
              <div className="relative aspect-[4/5] w-full overflow-hidden rounded-[1.5rem] bg-slate-900 shadow-inner">
                {avatarSrc ? (
                  <OptimizedImage
                    src={avatarSrc}
                    alt={profile.name}
                ...
                {/* Badge Note Flottant */}
                <div className="absolute top-3 right-3 flex items-center gap-1 rounded-full bg-black/60 px-3 py-1.5 text-xs font-black text-amber-300 backdrop-blur-md border border-white/10 shadow-sm z-20">
                ...
                <div className="absolute bottom-3 left-3 right-3 flex items-center justify-between z-20">
                  <span className="rounded-full bg-white/95 px-3 py-1 text-[10px] font-black text-purple-900 shadow-md backdrop-blur-sm uppercase tracking-widest">
                    {profile.category}
                  </span>
              ...
              <div className="pt-4 px-2">
                <div className="flex items-start justify-between gap-1 mb-2">
                  <div>
                    <div className="flex items-center gap-2 mb-0.5">
                      <h3 className="font-black text-xl text-slate-900 group-hover:text-purple-700 transition-colors tracking-tighter">
                        {profile.name}
                      </h3>
                ...
                <p className="text-xs text-slate-500 line-clamp-2 leading-relaxed mb-4 font-medium italic">
            ...
            <div className="pt-3 border-t border-slate-100 flex items-center gap-2.5 px-1">
              <Link href={`/profiles/${profile.id}`} className="flex-1">
                <button className="w-full flex items-center justify-center gap-1.5 rounded-2xl bg-slate-50 py-3 text-xs font-black uppercase tracking-tighter text-slate-700 hover:bg-purple-50 hover:text-purple-700 transition-all active:scale-95 border border-slate-100">
              ...
              <button
                ...
                className="flex-1 flex items-center justify-center gap-1.5 rounded-2xl bg-slate-950 py-3 text-xs font-black uppercase tracking-tighter text-white shadow-xl hover:bg-purple-600 transition-all active:scale-95"
              >
