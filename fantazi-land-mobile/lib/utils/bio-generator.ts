...
export function generatePersonalizedBio(data: BioData): string {
  ...
  // Deterministic seed-like logic for random-ish but stable selection if possible
  // For now simple random for variety
  const intro = intros[name.length % intros.length];
  const value = valueProps[category.length % valueProps.length];
